  ...
  constructor(services) {
    ...
    router.post(
      ...
      r(async (req, res, next) => {
        ...
        if (usersFound.error !== null)
          return res.status(500).send({ error: usersFound.error.message });
        ...
        if (userCreation.error !== null)
          return res.status(500).send({ error: userCreation.error.message });

        const userAuth = await w(
          userManager.authenticate({ email: email, password: password })
        );

        if (userAuth.error !== null)
          return res.status(409).send({ error: userAuth.error.message });
    ...
    router.get(
      "/user/:userId",
      r(async (req, res, next) => {
        const { result, error } = await w(
          userManager.getById(req.params.userId)
        );
